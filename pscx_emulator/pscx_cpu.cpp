...
{
	m_pc = (m_pc & 0xf0000000) | (instruction.getJumpTargetValue() << 2);
...
Cpu::InstructionType Cpu::opcodeBNE(const Instruction& instruction)
{
	...
	{
		branch(instruction.getSignExtendedImmediateValue(), m_pc);
	}
...
Cpu::InstructionType Cpu::opcodeLW(const Instruction& instruction)
{
	...
	uint32_t address = getRegisterValue(instruction.getRegisterSourceIndex()) + instruction.getSignExtendedImmediateValue();
	Instruction instructionLocal = load32(address);

	if ((instructionLocal.getInstructionStatus() == instruction.INSTRUCTION_STATUS_UNALIGNED_ACCESS) || (instructionLocal.getInstructionStatus() == instruction.INSTRUCTION_STATUS_UNHANDLED_FETCH))
		return INSTRUCTION_TYPE_UNKNOWN;
	if (instructionLocal.getInstructionStatus() == instruction.INSTRUCTION_STATUS_NOT_IMPLEMENTED)
		return INSTRUCTION_TYPE_NOT_IMPLEMENTED;
	m_load = RegisterData(instruction.getRegisterTargetIndex(), instructionLocal.getInstructionOpcode());
...
Cpu::InstructionType Cpu::opcodeSH(const Instruction& instruction)
{
	...
	store16(address, (uint16_t) getRegisterValue(instruction.getRegisterTargetIndex()));
...
Cpu::InstructionType Cpu::opcodeSB(const Instruction& instruction)
{
	...
	store8(address, (uint8_t)getRegisterValue(instruction.getRegisterTargetIndex()));
...
{
	uint32_t address = getRegisterValue(instruction.getRegisterSourceIndex()) + instruction.getSignExtendedImmediateValue();
	Instruction instructionLocal = load8(address); //cast to sign??
